...
// V6.7.0: Render inbox notifications terus dalam tab (untuk Pelulus)
function renderInboxAsTab() {
  const tabList = document.getElementById('applicationsList');
  if (!tabList) return;
  
  if (!cachedInboxData || cachedInboxData.length === 0) {
    tabList.innerHTML = '<div style="padding:40px; text-align:center; color:#94a3b8;"><div style="font-size:2.5rem; margin-bottom:10px;">📭</div><div style="font-size:1.1rem; font-weight:bold;">Tiada mesej dalam inbox</div></div>';
    return;
  }
  
  const searchVal = (document.getElementById('inboxSearchTab')?.value || '').toLowerCase().trim();
  
  let html = `<div style="margin-bottom:10px;"><input type="text" id="inboxSearchTab" placeholder="🔍 Cari syarikat / mesej..." style="width:100%; padding:10px 14px; border:1px solid #e5e7eb; border-radius:8px; font-size:0.95rem; box-sizing:border-box;"></div>`;
  ...
  cachedInboxData.forEach((msg, index) => {
    ...
    html += `
      <div class="inbox-item ${isUnread ? 'unread' : ''}" style="border:1px solid #e5e7eb; border-radius:10px; margin-bottom:8px; padding:14px; background:${isUnread ? '#f0f9ff' : '#fff'}; display:flex; align-items:flex-start; gap:12px; cursor:pointer;">
        <div style="font-size:1.3rem; margin-top:2px;">${icon}</div>
        <div style="flex:1; min-width:0;">
          <div style="font-weight:${isUnread ? '700' : '400'}; margin-bottom:4px; white-space:pre-line;">${msg.mesej}</div>
          <div style="display:flex; gap:8px; flex-wrap:wrap; font-size:0.8rem; color:#64748b;">
            <span class="inbox-badge ${badge}" style="padding:2px 8px; border-radius:4px; font-size:0.75rem;">${msg.jenisMsg}</span>
            ${msg.syarikat ? `<span>🏢 ${msg.syarikat}</span>` : ''}
            ${msg.jenis ? `<span>📋 ${msg.jenis}</span>` : ''}
            <span>⏱ ${masaStr}</span>
          </div>
        ...
        <div style="display:flex; gap:6px; flex-shrink:0;">
          ${msg.row ? `<button class="inbox-tab-proses" data-msgid="${msg.id}" data-row="${msg.row}" data-idx="${index}" style="padding:8px 16px; background:#2563eb; color:white; border:none; border-radius:8px; font-weight:600; cursor:pointer;">⚙️ Proses</button>` : ''}
          <button class="inbox-tab-delete" data-msgid="${msg.id}" data-row="${msg.row}" data-idx="${index}" style="padding:8px 12px; background:#fee2e2; color:#dc2626; border:none; border-radius:8px; cursor:pointer;">🗑</button>
        </div>
      </div>
    `;
  });
  
  if (!cachedInboxData.some((msg, i) => {
    if (searchVal) {
      return (msg.syarikat && msg.syarikat.toLowerCase().includes(searchVal)) ||
        (msg.mesej && msg.mesej.toLowerCase().includes(searchVal)) ||
        (msg.jenis && msg.jenis.toLowerCase().includes(searchVal));
    }
    return true;
  })) {
    tabList.innerHTML = '<div style="padding:40px; text-align:center; color:#94a3b8;">Tiada hasil carian.</div>';
    return;
  }
  ...
  // Search listener
  const searchInput = document.getElementById('inboxSearchTab');
  if (searchInput) {
    searchInput.addEventListener('input', () => renderInboxAsTab());
  }
  
  // Proses buttons
  tabList.querySelectorAll('.inbox-tab-proses').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      e.stopPropagation();
      const msgId = e.target.getAttribute('data-msgid');
      const row = e.target.getAttribute('data-row');
      ...
      if (msgId && row && cachedInboxData[idx] && !cachedInboxData[idx].dibaca) {
        try {
          await fetchWithRetry(SCRIPT_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'text/plain;charset=utf-8' },
            body: JSON.stringify({ action: 'markInboxRead', msgId, row, email: currentUser.email })
          }, 2, 500);
          cachedInboxData[idx].dibaca = true;
          renderInbox();
          renderInboxAsTab();
  ...
  // Delete buttons
  tabList.querySelectorAll('.inbox-tab-delete').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      e.stopPropagation();
      const msgId = e.target.getAttribute('data-msgid');
      const row = e.target.getAttribute('data-row');
      const idx = parseInt(e.target.getAttribute('data-idx'));
      const confirmed = await CustomAppModal.confirm('Padam mesej ini?', 'Padam', 'warning', 'Ya', true);
      if (!confirmed) return;
      try {
        const response = await fetchWithRetry(SCRIPT_URL, {
          method: 'POST',
          headers: { 'Content-Type': 'text/plain;charset=utf-8' },
          body: JSON.stringify({ action: 'deleteInbox', msgId, row, email: currentUser.email })
        }, 3, 1000);
        const result = await response.json();
        if (result.status === 'success') {
          cachedInboxData.splice(idx, 1);
  ...
  tabList.querySelectorAll('.inbox-item').forEach(item => {
    item.addEventListener('click', async (e) => {
      if (e.target.closest('button')) return;
      const idx = parseInt(item.querySelector('[data-idx]')?.getAttribute('data-idx'));
      if (isNaN(idx)) return;
      const msg = cachedInboxData[idx];
      if (!msg || msg.dibaca) return;
      try {
