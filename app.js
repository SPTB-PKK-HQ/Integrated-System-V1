...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // URL APPSCRIPT
  const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzvoT88Wnbj0u2FwoDoIF9e16qXFYZ8go1JrcXES-zNiVUbFL46VquLrVTAoz7XyaK0/exec';
...
async function handleCredentialResponse(response) {
  ...
  function updateTypeChartFromAgg(agg) {
    ...
    dashboardTypeChart = new Chart(ctx, {
      ...
      data: {
        labels: labels,
        datasets: [{ data: values, backgroundColor: ['#3b82f6', '#10b981', '#f59e0b', '#8b5cf6', '#64748b'], borderWidth: 3, borderColor: '#ffffff', hoverOffset: 15, borderRadius: 8 }]
      },
      options: {
        responsive: true,
        maintainAspectRatio: false,
  ...
  function updateTrendChartFromAgg() {
    ...
    const inProc = keys.map(k => isPengesyor
      ? (monthMap[k] ? Math.max(0, monthMap[k].total - (monthMap[k].sokong || 0) - (monthMap[k].tidakSokong || 0)) : 0)
          ...
          tooltip: {
            callbacks: {
              footer: (items) => {
                ...
                return 'JUMLAH: ' + totalData[items[0].dataIndex];
              }
            }
          }
        }
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              footer: (items) => {
                if (!items.length) return '';
                return 'JUMLAH: ' + (monthlyData[monthlyLabels[items[0].dataIndex]]?.total || 0);
              }
            }
          }
        }
...
Sila semak sistem STB untuk tindakan selanjutnya.`;
 ...
 async function logoutUserOnTimeout() {
  ...
  async function loadPengesyorState() {
    ...
    addPerson();
    // Bank: mulakan satu kad kosong (tanpa pulihkan data lama — ikut fix bocor data)
    clearBankCards();
    addBankCard();
