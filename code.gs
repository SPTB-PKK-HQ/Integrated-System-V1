...
function handleCetakDanSimpanPDF(data) {
  try {
    ...
    var themeColor = data.user_color && data.user_color.trim() !== "" ? data.user_color : "#6366f1";
...
function processPemutihanQueue() {
  ...
  q.forEach(function(d, i) {
    rows += '<tr><td style="padding:10px;border:1px solid #ddd;text-align:center;">' + (i+1) + '</td><td style="padding:10px;border:1px solid #ddd;"><strong>' + d.syarikat + '</strong></td><td style="padding:10px;border:1px solid #ddd;text-align:center;">' + d.cidb + '</td><td style="padding:10px;border:1px solid #ddd;text-align:center;">' + d.gred + '</td><td style="padding:10px;border:1px solid #ddd;">' + (d.alamat_perniagaan||'Tiada') + '</td><td style="padding:10px;border:1px solid #ddd;">' + (d.justifikasi||'Tiada') + '</td><td style="padding:10px;border:1px solid #ddd;text-align:center;">' + (d.pelulus||'Tiada') + '</td><td style="padding:10px;border:1px solid #ddd;text-align:center;"><a href="' + d.pautan + '" style="color:#6366f1;">Buka</a></td></tr>';
...
function processSiasatQueue() {
  ...
  var rows = '', text = '';
  q.forEach(function(d, i) {
    rows += '<tr><td style="padding:10px;border:1px solid #ddd;text-align:center;">' + (i+1) + '</td><td style="padding:10px;border:1px solid #ddd;"><strong>' + d.syarikat + '</strong></td><td style="padding:10px;border:1px solid #ddd;text-align:center;">' + d.cidb + '</td><td style="padding:10px;border:1px solid #ddd;text-align:center;">' + d.gred + '</td><td style="padding:10px;border:1px solid #ddd;">' + (d.alamat_perniagaan||'Tiada') + '</td><td style="padding:10px;border:1px solid #ddd;">' + (d.justifikasi||'Tiada') + '</td><td style="padding:10px;border:1px solid #ddd;text-align:center;">' + d.pengesyor + '</td><td style="padding:10px;border:1px solid #ddd;text-align:center;"><a href="' + d.pautan + '" style="color:#6366f1;">Buka</a></td></tr>';
    text += (i+1) + '. ' + d.syarikat + '\n   CIDB: ' + d.cidb + ' | Gred: ' + d.gred + ' | Pengesyor: ' + d.pengesyor + '\n\n';
  });
  try {
    MailApp.sendEmail({ to: EMAIL_TO_SPI, cc: EMAIL_CC_SPTB, subject: 'Makluman Harian: ' + q.length + ' Permohonan SPI', htmlBody: '<!DOCTYPE html><html><head><style>body{font-family:sans-serif}table{width:100%;border-collapse:collapse}th{background:#6366f1;color:white;padding:10px}</style></head><body><h2>📋 MAKLUMAN HARIAN SPI</h2><table><thead><tr><th>Bil</th><th>Syarikat</th><th>CIDB</th><th>Gred</th><th>Alamat</th><th>Justifikasi</th><th>Pengesyor</th><th>Drive</th></tr></thead><tbody>' + rows + '</tbody></table><p>*** Emel automatik ***</p></body></html>', body: 'NOTIS HARIAN SPI\n\n' + text + '*** Emel automatik ***', name: EMAIL_SENDER_NAME });
    updateSPIStatusInSheet(q);
...
function testCetakDanSimpanPDF() { return handleCetakDanSimpanPDF({ htmlContent: '<div>Test</div>', company_name: 'TEST', user_name: 'Test', application_type: 'BARU', user_color: '#6366f1' }); }
